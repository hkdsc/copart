...
def get_rotate_camera_batch(num_rotate, fovy = 0.8575560450553894, iter_res=[512,512], cam_near_far=[0.1, 1000.0], cam_radius=1.2, elevation=30, azimuth=[0, 2*np.pi], device="cuda", rot_yx_90=False):
    azimuth = torch.linspace(azimuth[0], azimuth[1], num_rotate+1)[:-1]
    elevation = torch.tensor(np.deg2rad(elevation)).repeat(num_rotate)
    x, y, z = kal.ops.coords.spherical2cartesian(azimuth, elevation, -torch.tensor(cam_radius))
    if rot_yx_90:
        x, y, z = y, -z, x
    camera_pos = torch.stack((x, y, z), dim=-1)
    return kal.render.camera.Camera.from_args(
        eye=camera_pos,
        at=torch.zeros(num_rotate, 3),
        up=torch.tensor([0., 1., 0.]),
        fov=fovy,
        near=cam_near_far[0], far=cam_near_far[1],
        height=iter_res[0], width=iter_res[1],
...
def render_mesh(mesh, camera, iter_res, return_types = ["mask", "depth"], white_bg=False, wireframe_thickness=0.4):
    ...
    proj = camera.projection_matrix().unsqueeze(1)#.clone()
    proj[:, :, 1, 1] = -proj[:, :, 1, 1]
    ...
    for type in return_types:
        if type == "mask" :
            ...
            img = dr.interpolate(
                ...
                torch.arange(mesh.faces.shape[0] * 3, device=mesh.face_normals.device, dtype=torch.int).reshape(-1, 3)
            )[0]
        elif type == "tex_pos":
